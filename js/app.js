...
(async function () {
  ...
  async function renderHome() {
    ...
    const total = Lessons.getTotalLessonsCount();
    const percent = Progress.getPercent(total);
  ...
  document.getElementById("exercise-check").onclick = () => {
    ...
    const currentData = activeLesson.exercises[exerciseIndex];
    ...
    if (correct) {
      ...
      if (currentData.type === "write") {
        feedback.textContent = `Не совсем. Верный ответ: ${currentData.answers[0]}`;
      } else {
  ...
  async function finishLesson() {
    ...
    document.getElementById("complete-stats").textContent =
      `Правильно: ${correctCount} из ${activeLesson.exercises.length} · Курс пройден на ${percent}%`;
    showScreen("complete");
  }
